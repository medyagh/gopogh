...
function createRecentNumberOfFailTable(summaryTable) {
  ...
  const table = document.createElement("table");
    ...
    for (let i = 0; i < summaryTable.length; i++) {
        const { envName, recentNumberOfFail, growth } = summaryTable[i];
...
function createRecentFlakePercentageTable(recentFlakePercentTable, query) {
  ...
  function displayTestAndEnvironmentChart(data, query) {
    ...
    dayChart.addColumn({ type: 'string', role: 'tooltip', 'p': { 'html': true } });
    ...
    dayChart.addRows(
      dayData
        .map(groupData => {
          ...
          const resultArr = dataArr.map((commit) => ({id: commit[commit.length -3], status: (commit[commit.length - 2]).trim()}))
          const durationArr = dataArr.map((commit) => ({id: commit[commit.length -3], status: (commit[commit.length - 2]).trim(), duration : (commit[commit.length -1]).trim()}))
    ...
    weekChart.addColumn({ type: 'string', role: 'tooltip', 'p': { 'html': true } });
  ...
  function displaySummaryChart(data) {
    ...
    for (const envDay of summaryData) {
      const {startOfDate, envName, avgFailedTests, avgDuration} = envDay
      uniqueDayDates.add(startOfDate)
      if (!summaryEnvDateMap[envName]) {
        summaryEnvDateMap[envName] = {};
      }
      summaryEnvDateMap[envName][startOfDate] = {avgFailedTests, avgDuration}
    }
    ...
    dayChart.addRows(orderedDayDates.map(dateTime => [new Date(dateTime)].concat(uniqueEnvs.map(name => {
      ...
      if (avgVal !== undefined) {
        const {avgFailedTests, avgDuration} = avgVal
        return [
          avgFailedTests,
          `<div style="padding: 1rem; font-family: 'Arial'; font-size: 14">
    ...
    const dayOptions = {
      ...
      pointShape: "circle",
      vAxes: {
        0: { title: "# of Failed Tests", minValue: 0},
      },
      tooltip: { trigger: "selection", isHtml: true }
    };
    ...
    for (const env of uniqueEnvs) {
      ...
      durChart.addColumn({ type: 'string', role: 'tooltip', 'p': { 'html': true } });
    }
    ...
    const durOptions = {
      ...
      pointShape: "circle",
      vAxes: {
        0: { title: "Total Duration", minValue: 0},
      },
      tooltip: { trigger: "selection", isHtml: true }
    };
    // Create the chart and draw it
  ...
  function displayEnvironmentChart(data, query) {
    ...
    dayData.forEach((day) => {
      const { testName, startOfDate, flakePercentage, commitResults} = day;
      ...
      flakeDayDataMap[testName][startOfDate] = {fp : flakePercentage, cr : commitResults};
    });
    const dayChart = new google.visualization.DataTable();
    dayChart.addColumn('date', 'Date');
    ...
    dayChart.addRows(orderedDayDates.map(dateTime => [new Date(dateTime)].concat(uniqueDayTestNamesArray.map(name => {
      ...
      if (fpAndCr !== undefined) {
        ...
        commitArr = commitArr.map((commit) => ({id: commit[commit.length -2], status: (commit[commit.length - 1]).trim()}))
        return [
          fp,
          `<div style="padding: 1rem; font-family: 'Arial'; font-size: 14">
            <b style="display: block">${name}</b><br>
    ...
    weekData.forEach((week) => {
      const { testName, startOfDate, flakePercentage, commitResults } = week;
      // If the test name doesn't exist in the map, create a new entry
      if (!flakeWeekDataMap[testName]) {
        flakeWeekDataMap[testName] = {};
      }
      // Set the flakePercentage for the corresponding startOfDate
      flakeWeekDataMap[testName][startOfDate] = {fp: flakePercentage, cr: commitResults};
    });
    {
...

  {
    ...
    durationChart.addColumn({ type: 'string', role: 'tooltip', 'p': { 'html': true } });
    ...
    durationChart.addRows(
      ...
      countArr = countArr.map((commit) => ({rootJob: commit[commit.length -2], testCount: +(commit[commit.length - 1]).trim()}))
      let durationArr = dateInfo.commitDurations.split(",")
      durationArr  = durationArr.map((commit) => commit.split(":"))
      durationArr = durationArr.map((commit) => ({rootJob: commit[commit.length -2], totalDuration: +(commit[commit.length - 1]).trim()}))
        return [
  ...
  async function init() {
    ...
    const desiredTest = query.test, desiredEnvironment = query.env, desiredPeriod = query.period || "", desiredTestNumber = query.tests_in_top || "";
